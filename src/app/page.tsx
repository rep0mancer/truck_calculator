...
const TRUCK_TYPES: any = {
  curtainSider: {
    ...
    usableLength: 1320,
    maxWidth: 245,
    // capacity reference targets
    singleLayerEUPCapacity3Across: 33, // 11 rows × 3 across (120×80)
    singleLayerDINCapacity: 26,        // 13 rows × 2 across (100-step)
...
/* ----------------------------------------------------------------
   CORE PLANNER – strictly per your rules
----------------------------------------------------------------- */
...
type PlanInputs = {
  ...
  eupStackLimit?: number; // number of pallets that may be stacked
  dinStackLimit?: number; // number of pallets that may be stacked
  eupWeightStr: string;
  dinWeightStr: string;
...
const calculatePlan = (input: PlanInputs): PlanResult => {
  const {
    truckKey,
    reqEUP,
    reqDIN,
  ...
  // DIN rows: true DIN raster → step 100 along length, two slots across (120 each)
  const placeDINRows = (stacked: boolean) => {
    ...
    for (const unit of units) {
      ...
      while (unit.currentX + 100 <= unit.length && dinLeft > 0) {
        unit.currentY = 0;
        let placedInRow = 0;

        for (let i = 0; i < 2 && dinLeft > 0; i++) {
          if (!canAdd(ctx.dinWeight)) {
          ...
          if (unit.currentY + 120 <= unit.width) {
            ...
            const base = {
              x: unit.currentX,
              y: unit.currentY,
              width: 100,   // along length
              height: 120,  // across width
              type: "din",
        ...
        if (placedInRow > 0) unit.currentX += 100;
        else break;
      }
    }
  };

  // EUP placement: try “broad” (3 across, step 120) vs “long” (hybrid: one 3-across row + 2 across step 80)
  const placeEUP = (stacked: boolean, pattern: EupPattern) => {
    if (eupLeft <= 0) return;

    const tryOrders: EupPattern[] = pattern === "auto" ? ["broad", "long"] : [pattern];

    const scoreAttempt = (unitsAttempt: any[]) => {
      let count = 0;
      for (const u of unitsAttempt) count += u.palletsVisual.length;
      return count;
    };

    let bestUnits: any[] = [];
    let bestCtx: PlaceCtx | null = null;
    let bestEupLeft = eupLeft;
    let bestScore = -1;

    for (const variant of tryOrders) {
      // deep clone working state
      const cloneUnits = JSON.parse(JSON.stringify(units));
      const localCtx: PlaceCtx = JSON.parse(JSON.stringify(ctx));
      let left = eupLeft;
      ...
      if (variant === "broad") {
        ...
        for (const unit of cloneUnits) {
          if (left <= 0) break;
          while (unit.currentX + 120 <= unit.length && left > 0) {
            unit.currentY = 0;
            let placed = 0;
            for (let i = 0; i < 3 && left > 0; i++) {
              if (!(localCtx.weightLimit <= 0 || localCtx.totalWeight + localCtx.eupWeight <= localCtx.weightLimit)) {
                unit.currentX = unit.length; break;
              }
              if (unit.currentY + 80 <= unit.width) {
                ...
                const base = {
                  ...
                  type: "euro",
                  isStackedTier: null as any,
                  key: `eup_broad_base_${unit.id}_${baseId}_${i}`,
                  unitId: unit.id,
                  labelId: baseId,
                  displayBaseLabelId: baseId,
                  displayStackedLabelId: null as any,
                ...
                if (stacked && left > 0 && localCtx.eupTopsBudget > 0 &&
                  ...
                  unit.palletsVisual.push({
                    ...base,
                    isStackedTier: "top" as const,
                    key: `eup_broad_top_${unit.id}_${topId}_${i}`,
                    labelId: topId,
                    displayBaseLabelId: baseId,
                    displayStackedLabelId: topId,
                    showAsFraction: true,
                  ...
                  localCtx.totalWeight += localCtx.eupWeight;
                  left--;
                  localCtx.eupTopsBudget--;
                }

                unit.currentY += 80;
              }
            }
            if (placed > 0) unit.currentX += 120;
            else break;
          }
        }
      } else {
        // "long" (Längsverladung): mostly 2 across, step 80 + one 3-across row (120) if it improves result
        for (const unit of cloneUnits) {
          if (left <= 0) break;

          // If length allows, insert one 3-across row up front
          if (unit.length - unit.currentX >= 120 && left > 0) {
            unit.currentY = 0;
            let placed = 0;
            for (let i = 0; i < 3 && left > 0; i++) {
              if (!(localCtx.weightLimit <= 0 || localCtx.totalWeight + localCtx.eupWeight <= localCtx.weightLimit)) { unit.currentX = unit.length; break; }
              if (unit.currentY + 80 <= unit.width) {
                const baseId = ++localCtx.eupLabel;
                const base = {
                  x: unit.currentX, y: unit.currentY, width: 120, height: 80, type: "euro",
                  isStackedTier: null as any, key: `eup_long_hybrid_${unit.id}_${baseId}_${i}`,
                  unitId: unit.id, labelId: baseId, displayBaseLabelId: baseId, displayStackedLabelId: null as any, showAsFraction: false
                };
                unit.palletsVisual.push(base);
                unit.occupiedRects.push({ x: base.x, y: base.y, width: base.width, height: base.height });
                localCtx.totalAreaBase += PALLET_TYPES.euro.length * PALLET_TYPES.euro.width;
                ...
                if (stacked && left > 0 && localCtx.eupTopsBudget > 0 &&
                  ...
                  unit.palletsVisual.push({
                    ...base, isStackedTier: "top" as const, key: `eup_long_hybrid_top_${unit.id}_${topId}_${i}`,
          ...
          // 2 across, step 80 (rotated 120×80)
          while (unit.currentX + 80 <= unit.length && left > 0) {
            unit.currentY = 0;
            let placed = 0;
            for (let i = 0; i < 2 && left > 0; i++) {
              ...
              if (unit.currentY + 120 <= unit.width) {
                const baseId = ++localCtx.eupLabel;
                const base = {
                  x: unit.currentX, y: unit.currentY, width: 80, height: 120, type: "euro",
                  isStackedTier: null as any, key: `eup_long_base_${unit.id}_${baseId}_${i}`,
                  unitId: unit.id, labelId: baseId, displayBaseLabelId: baseId, displayStackedLabelId: null as any, showAsFraction: false
                };
                unit.palletsVisual.push(base);
                unit.occupiedRects.push({ x: base.x, y: base.y, width: base.width, height: base.height });
                ...
                if (stacked && left > 0 && localCtx.eupTopsBudget > 0 &&
                  ...
                  unit.palletsVisual.push({
                    ...base, isStackedTier: "top" as const, key: `eup_long_top_${unit.id}_${topId}_${i}`,
                    labelId: topId, displayBaseLabelId: baseId, displayStackedLabelId: topId, showAsFraction: true
                  });
                  localCtx.totalWeight += localCtx.eupWeight; left--; localCtx.eupTopsBudget--;
                }
            ...
            if (placed > 0) unit.currentX += 80; else break;
          }
        }
      }

      // evaluate
      const score = scoreAttempt(cloneUnits);
      if (score > bestScore) {
        bestScore = score;
        bestUnits = cloneUnits;
        bestCtx = localCtx;
        bestEupLeft = left;
      }
    }
    ...
    // commit best
    if (bestCtx) {
      // overwrite units state in-place
      for (let i = 0; i < units.length; i++) {
        units[i] = bestUnits[i];
      }
      ctx.eupLabel = bestCtx.eupLabel;
      ctx.eupTopsBudget = bestCtx.eupTopsBudget;
      ctx.totalAreaBase = bestCtx.totalAreaBase;
      ctx.totalWeight = bestCtx.totalWeight;
      eupLeft = bestEupLeft;
    }
  };

  /* ---------------------- PIPELINE ---------------------- */
  // DIN first: place DIN, micro-fill single-slot with one long EUP, then EUP, DIN, EUP
  const runDinFirst = () => {
    const snapshotUnits = JSON.parse(JSON.stringify(units));
    const snapshotCtx = JSON.parse(JSON.stringify(ctx));
    let snapEupLeft = eupLeft;
    let snapDinLeft = dinLeft;

    // 1) stacked DIN
    placeDINRows(true);

    // micro-fill: if last DIN row ended single, fill partner slot with one long EUP
    if (snapEupLeft > 0) {
      for (const u of units) {
        const xRowStart = Math.max(0, u.currentX - 100);
        const rowDin = u.palletsVisual.filter((p: any) => p.type === "din" && p.x === xRowStart && !p.showAsFraction);
        if (rowDin.length === 1 && eupLeft > 0) {
          ...
          eupLeft--;
          if (eupStackable && ctx.eupTopsBudget > 0 && eupLeft > 0 && canAdd(ctx.eupWeight)) {
    ...
    // 2) stacked EUP
    placeEUP(true, eupPattern);
    // 3) remaining DIN (unstacked)
    placeDINRows(false);
    // 4) remaining EUP
    placeEUP(false, eupPattern);

    return { snapshotUnits, snapshotCtx, snapEupLeft, snapDinLeft };
  };

  // EUP first: EUP, DIN, EUP, DIN; micro-fill at end if needed
  const runEupFirst = () => {
    const snapshotUnits = JSON.parse(JSON.stringify(units));
    const snapshotCtx = JSON.parse(JSON.stringify(ctx));
    let snapEupLeft = eupLeft;
    ...
    // micro-fill after last DIN row if one slot free and some EUP left
    if (eupLeft > 0) {
      for (const u of units) {
        const xRowStart = Math.max(0, u.currentX - 100);
        const rowDin = u.palletsVisual.filter((p: any) => p.type === "din" && p.x === xRowStart && !p.showAsFraction);
  ...
  // try both orders; pick the one that satisfies counts, else the one that places more
  const cloneState = () => ({
  ...
  // restore and try B
  for (let i = 0; i < orig.units.length; i++) units[i] = orig.units[i];
  Object.assign(ctx, orig.ctx);
  eupLeft = orig.eupLeft;
  dinLeft = orig.dinLeft;

  const B = runEupFirst();
  const afterB = { units, ctx, eupLeft, dinLeft };

  const countPlaced = (arrUnits: any[]) => arrUnits.reduce((s: number, u: any) => s + u.palletsVisual.length, 0);
  const aFull = afterA.eupLeft === 0 && afterA.dinLeft === 0;
  const bFull = afterB.eupLeft === 0 && afterB.dinLeft === 0;

  let finalUnits = afterA.units;
  let finalCtx = afterA.ctx;
  let finalEupLeft = afterA.eupLeft;
  let finalDinLeft = afterA.dinLeft;

  if (bFull && !aFull) {
    finalUnits = afterB.units; finalCtx = afterB.ctx; finalEupLeft = afterB.eupLeft; finalDinLeft = afterB.dinLeft;
  } else if (!aFull && !bFull && countPlaced(afterB.units) > countPlaced(afterA.units)) {
    finalUnits = afterB.units; finalCtx = afterB.ctx; finalEupLeft = afterB.eupLeft; finalDinLeft = afterB.dinLeft;
  }
  ...
  const pallets = finalUnits.flatMap((u: any) => u.palletsVisual);
  ...
  const usedArea = finalCtx.totalAreaBase;
  ...
  const weightPerMeter = usedLength > 0 ? finalCtx.totalWeight / (usedLength / 100) : 0;
  if (weightPerMeter >= MAX_WEIGHT_PER_METER_KG) {
    finalCtx.warnings.push(`ACHTUNG – mögliche Achslastüberschreitung: ${weightPerMeter.toFixed(1)} kg/m`);
  }

  const stackedEup = totalEuro - euroBase;
  const stackedDin = totalDin - dinBase;
  if (stackedEup >= AXLE_WARN_EUP_STACKED)
    finalCtx.warnings.push(`ACHTUNG - ACHSLAST bei EUP im AUGE BEHALTEN! (${stackedEup} gestapelte EUP)`);
  if (stackedDin >= AXLE_WARN_DIN_STACKED)
    finalCtx.warnings.push(`ACHTUNG - ACHSLAST bei DIN im AUGE BEHALTEN! (${stackedDin} gestapelte DIN)`);

  finalCtx.warnings = Array.from(new Set(finalCtx.warnings));

  return {
    palletArrangement: finalUnits.map((u: any) => ({
      unitId: u.id,
      unitLength: u.length,
      unitWidth: u.width,
    ...
    totalWeightKg: finalCtx.totalWeight,
    eupLoadingPatternUsed: eupPattern,
  };
};
...
export default function Page() {
  ...
  const [eupPattern, setEupPattern] = useState<EupPattern>("auto"); // 3-across preferred, long variant on request
  const [eupStackable, setEupStackable] = useState(false);
  const [dinStackable, setDinStackable] = useState(false);

  // keep as strings to avoid leading zeros issues
  ...
  const renderPallet = (p: any, scale = 0.3) => {
    ...
    const w = p.height * scale; // horizontal in canvas
    const h = p.width * scale;  // vertical in canvas
    const x = p.y * scale;
    const y = p.x * scale;

    const label =
      p.showAsFraction && p.displayStackedLabelId
      ...
      <main className="p-6 bg-white shadow-lg rounded-b-lg">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="lg:col-span-1 space-y-6 bg-slate-50 p-5 rounded-lg border border-slate-200 shadow-sm">
            ...
            <div>
              <button onClick={handleFillRestEUP} className="w-full py-2 px-4 bg-indigo-600 text-white font-semibold rounded-md shadow-sm hover:bg-indigo-700">
            ...
            <div className="border-t pt-4">
              ...
              <div className="border-t pt-4">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  EUP Lade-Pattern:
                  <span className="text-xs text-gray-500"> (Gewählt: {actualPattern})</span>
                </label>
