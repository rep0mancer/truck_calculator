...
export default function HomePage() {
  ...
  const calculateAndSetState = useCallback(() => {
    ...
    if (dinQuantity > 0 && eupQuantity === 0) {
        ...
        const eupCapacityResult = calculateLoadingLogic(selectedTruck, MAX_PALLET_SIMULATION_QUANTITY, 0, isEUPStackable, isDINStackable, eupWeightPerPallet, dinWeightPerPallet, eupLoadingPattern, 'DIN_FIRST', eupStackLimit, dinStackLimit);
  ...
  const handleClearAllPallets = () => {
    ...
    toast({ title: 'Zurückgesetzt', description: 'Alle Eingaben wurden zurückgesetzt.' });
  };
  ...
  const suggestFeasibleLoad = () => {
    toast({ title: 'Funktion nicht implementiert', description: 'Diese Funktion ist derzeit nicht verfügbar.' });
  };

  const handleMaximizePallets = (typeToMax) => {
    const res = calculateLoadingLogic(
        selectedTruck,
        typeToMax === 'euro' ? MAX_PALLET_SIMULATION_QUANTITY : eupQuantity,
        typeToMax === 'industrial' ? MAX_PALLET_SIMULATION_QUANTITY : dinQuantity,
        isEUPStackable, isDINStackable,
        eupWeightPerPallet, dinWeightPerPallet,
        'auto', 'DIN_FIRST',
    ...
    toast({ title: 'Maximiert', description: `Maximale ${typeToMax.toUpperCase()} Paletten wurden eingestellt.` });
  };
  ...
  const handleFillRemaining = (typeToFill) => {
    ...
    const addedEups = res.totalEuroPalletsVisual - eupQuantity;
    const addedDins = res.totalDinPalletsVisual - dinQuantity;

    if (typeToFill === 'euro' && addedEups > 0) {
        setEupQuantity(eupQuantity + addedEups);
    } else if (typeToFill === 'industrial' && addedDins > 0) {
        setDinQuantity(dinQuantity + addedDins);
    }
