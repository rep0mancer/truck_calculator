...
import { calculateLoadingLogic as calculate, MAX_PALLET_SIMULATION_QUANTITY, PALLET_TYPES, TRUCK_TYPES, type EupPattern, type LoadingWarning, type PlacementOrder, type TruckKey, type PalletRectangle } from '@/lib/loading';

type WeightEntry = { id: number; weight: string; quantity: number };
const MAX_GROSS_WEIGHT_KG = 24000;
const KILOGRAM_FORMATTER = new Intl.NumberFormat('de-DE', { maximumFractionDigits: 0 });
const localizeWarning = (warning: LoadingWarning): string => {
  const v = warning.values;
  switch (warning.code) {
    case 'PALLETS_REJECTED': return `Konnte nicht alle Paletten laden. Übrig: ${v.industrial} DIN und ${v.euro} EUP.`;
    case 'PAYLOAD_REACHED': return `Gewichtslimit von ${v.limitKg} kg erreicht.`;
    case 'STACKING_DISABLED': return 'Info: Stapeln ist auf dem Waggon nicht möglich und wurde deaktiviert.';
    case 'DIN_CAPACITY_REACHED': return `Maximale DIN-Kapazität ist ${v.limit}.`;
    case 'AXLE_DENSITY': return `ACHTUNG – mögliche Achslastüberschreitung: ${v.kgPerMeter} kg/m`;
    case 'CONCENTRATED_LOAD': return 'ACHTUNG – konzentrierte Ladung.';
  }
};
const calculateLoadingLogic = (truckKey: TruckKey, eup: WeightEntry[], din: WeightEntry[], euroStackable: boolean, industrialStackable: boolean, eupPattern: EupPattern, placementOrder: PlacementOrder = 'DIN_FIRST', maxStackedEuro: number | string = 0, maxStackedIndustrial: number | string = 0) => {
  const groups = (entries: WeightEntry[]) => entries.map(e => ({ id: e.id, quantity: e.quantity, weightKg: e.weight.trim() === '' ? 0 : Number(e.weight) }));
  const result = calculate({ truckKey, euro: groups(eup), industrial: groups(din), euroStackable, industrialStackable, eupPattern, placementOrder, maxStackedEuro: Number(maxStackedEuro), maxStackedIndustrial: Number(maxStackedIndustrial) });
  return { ...result, warnings: result.warnings.map(localizeWarning) };
};
...
export default function HomePage() {
  ...
  const [palletArrangement, setPalletArrangement] = useState<ReturnType<typeof calculateLoadingLogic>['palletArrangement']>([]);
  ...
  const maxGrossWeightKg = selectedTruckConfig.maxPayloadKg ?? MAX_GROSS_WEIGHT_KG;
  ...
  const renderPallet = (pallet: PalletRectangle, displayScale = 0.3) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          <div className="lg:col-span-1 space-y-6 bg-slate-50 p-5 rounded-lg border border-slate-200 shadow-sm">
            ...
            <div className="border-t pt-4">
                ...
                <WeightInputs entries={dinWeights} onChange={(entries)=>{ setLastEdited('din'); setDinWeights(entries); }} palletType="DIN" preferredId={null} onSetPreferred={() => undefined} groupName="DIN" />
            ...
            <div className="border-t pt-4">
                ...
                <WeightInputs entries={eupWeights} onChange={(entries)=>{ setLastEdited('eup'); setEupWeights(entries); }} palletType="EUP" preferredId={null} onSetPreferred={() => undefined} groupName="EUP" />
          ...
          <div className="lg:col-span-2 bg-gray-100 p-6 rounded-lg border border-gray-200 shadow-sm flex flex-col items-center justify-center">
            ...
            {palletArrangement.map((unit,index: number)=>(
