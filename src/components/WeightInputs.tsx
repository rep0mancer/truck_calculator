...
export function WeightInputs({ entries, onChange, palletType, preferredId = null, onSetPreferred = () => undefined }: WeightInputsProps) {
  const { t } = useLocale();
  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-2">
        <label className="w-20 text-center text-xs font-semibold text-slate-600">{t('weights.quantity')}</label>
        <label className="w-32 text-center text-xs font-semibold text-slate-600">{t('weights.weight', { type: palletType })}</label>
      </div>
      {entries.map(entry => (
        <div key={entry.id} className="flex items-center gap-3 mt-2 rounded-2xl border border-white/30 bg-white/10 px-3 py-2 backdrop-blur-xl shadow-[0_18px_36px_-28px_rgba(15,23,42,0.45)]">
          <div className="flex items-center gap-2">
            ...
            <Input
              ...
              placeholder={t('weights.quantity')}
          ...
          <Input
            ...
            placeholder={t('weights.weightPlaceholder', { type: palletType })}
